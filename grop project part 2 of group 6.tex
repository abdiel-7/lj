...
\vspace*{\stretch{1}}
\begin{center}
  {\huge\bfseries Report about group project part 2}                  \\[6.5ex]
  {\large\bfseries Group six: Zihan Lin,Kevin Tran,Brice Arrigo \& Jude Hine}           \\
  \today
\end{center}
\vspace{\stretch{2}}
\end{titlepage}
...
\newpage
\section{Generate a dataset consisting of three classes of point clouds}

The code below generate a point cloud containing 200 points, sampling two disjoint concentric noisy circles.
\begin{lstlisting}
...
The code below generate a point cloud containing 200 points, sampling two disjoint noisy circles that are not nested.
\begin{lstlisting}
...
The code below generate a point cloud containing 200 points, sampling two adjacent noisy circles.
\begin{lstlisting}
...
And then in the "dataset" will contain three classes of point clouds.
\begin{lstlisting}
dataset = []
for i in range(0,3):
...
\section{Cluster the dataset}

Through persistentence barcode, we could see the difference in persistent homology among the three classes. 

The picture below is persistentence barcode of the point cloud sampling two disjoint concentric noisy circles.
\begin{figure}[h] 
\centering 
\includegraphics[width=1\textwidth]{1} 
\end{figure}

Its dim-1 barcodes are obviously differ from other two classes whose dim-1 barcodes are just two lines.

\newpage
The picture below is persistentence barcode of the point cloud sampling two adjacent noisy circles.
...
It only have one connected component. So other two classes will have more the one long dim-0 barcodes, while two adjacent noisy circles only have 1.

\newpage
The picture below is persistentence barcode of the point cloud sampling two disjoint noisy circles that are not nested.
...
The persistentence barcodes show that persistent homology could help us cluster the dataset.
...
\end{lstlisting}
\newpage
ph() is just a function to turn persistent homology into vectors for using of  KMeans.
...
This code will output one hundred 1, one hundred 0,  one hundred 2.

 We have generated 100 two disjoint concentric noisy circles firstly, then generated 100 two disjoint noisy circles that are not nested before generating 100 two adjacent noisy circles. So this code cluster the dataset successfully.

\section{Noise Influence}

We have set noise=0.1. If the noise is too big, this code will fail. 

If we set noise=0.3, it will still work (although not so correct). But if this increase to 0.5, it can not cluster the dataset anymore. That is because that the distance between two disjoint concentric noisy circles or two disjoint noisy circles that are not nested are both 1. So when noise is over 0.5, the difference in persistence homologies among them will be destoried. For example, two connected components may be considered as one now, which will influnce $PH_0$.

 If we still increase the noise, the persistent homologies of three classes will seem to be same for this code. 

 \section{A noisy circle and addition of “outlier” points}
The code below generate a point cloud P1 consisting of 200 points, sampling a noisy circle and create a second point cloud P2 by adding 20 “outlier” points to P1. It will also show their persistence diagrams
\begin{lstlisting}
...
P1 is a noisy circle, with only one connected component and one cycle. So only 1-dim of $PH_0$ and $PH_1$ will live for a long time. But these outlier points will lead to bigger long  living persistent homology for that some of them may be far from the circle, forming new connected components and edges when filtration index is small.

Persistence diagram of P1 is as follows.
...
The following persistence diagram is P2's. The influence of outlier points is obvious.
\begin{figure}[h] 
\centering 
\includegraphics[width=1\textwidth]{6} 
\end{figure}

 \section{Recover influence of “outlier” points}

As we have mentioned above,  the influence derive from some of outlier points far from the circle. So we just need to remove points far from other points.

 Let neighbour of point $a$ be the area where the distance between $a$ and all points in this area is less than 0.3. If a point's neighbour only cotain less than five points, we remove this "outlier" point (although it may not be the 20 "outlier" points we added before).
...
P2\_clean is the "recovered" point could. Its persistence diagram is:
...
It is similar to persistence diagram of P1. We successfully correct the behaviour of P2.
\end{document}
